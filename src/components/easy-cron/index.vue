        ...
        <div class="right">
          <div class="field-list">
            ...
            <Table stripe :columns="columns" :data="tableData"
                   :show-header="false" size="small"></Table>
          ...
          <div class="exe-pre">
            ...
            <div class="exe-pre-panel">
              <label class="p-left">执行预览</label>
              <el-input type="textarea" :value="preTimeList" class="p-right" :rows="15" readonly />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    cronValue_c () {
      let result = []
      if (!this.hideSecond) result.push(this.second ? this.second : '00')
  ...
  methods: {
    handleClose(){
      this.$emit('handleClose',this.timeData)
    },
    formatValue () {
